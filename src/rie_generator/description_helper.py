...
from rie_generator.core import MaskPolicy
def emulate_with_split_lmul(operation: Operation, generator: Callable, generator_extra_args: list, generator_extra_kwargs: dict) -> Operation:
    ...
    half_lmul = LMULType.divide(operation.node_format.lmul, 2)
    assert LMULType.is_valid_for_eew(operation.node_format.elt_type, half_lmul) 
    ...
    half_lmul_result_fmt = NodeFormatDescriptor(NodeFormatType.PLACEHOLDER, vd.node_format.elt_type, half_lmul)
    ...
    args_lo, args_hi = unzip(split_operands)
    ...
    result_elt = operation.node_format.elt_type
    full_result_fmt = NodeFormatDescriptor(NodeFormatType.VECTOR, result_elt, operation.node_format.lmul)
